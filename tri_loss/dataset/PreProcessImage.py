...
class PreProcessIm(object):
  def __init__(
    ...
    if purpose == 'train':
      self.cropper=transforms.Compose([
          transforms.Resize(tuple(map(lambda x: x * 9 // 8, resize_h_w))),
          transforms.RandomCrop(resize_h_w)])
    else:
      self.cropper=transforms.Compose([
          transforms.Resize(tuple(map(lambda x: x * 9 // 8, resize_h_w))),
          transforms.CenterCrop(resize_h_w)])
  ...
  def pre_process_im(self, im):
    ...
    im=np.asarray(self.cropper(Image.fromarray(im)))
